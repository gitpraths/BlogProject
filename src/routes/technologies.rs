...
pub async fn fetch_technology_news() -> impl Responder {
    match fetch_from_newsapi().await {
        Ok(articles) => {
          ...
          <body>
              ...
              <header>
                  <h1 id="sections">Technology News</h1>
                  </header>
      "##); 
        ...
        Err(err) => {
            eprintln!("Error fetching technology news: {:?}", err);
            HttpResponse::InternalServerError().body(format!("Failed to fetch technology news: {:?}", err))
        }
    }
}