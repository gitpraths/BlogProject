...
async fn fetch_entertainment_news() -> Result<Vec<Article>, reqwest::Error> {
...
pub async fn display_entertainment() -> impl Responder {
  match fetch_entertainment_news().await {
      Ok(articles) => {
          let mut html_content = String::from(r##"
              ...
              <body>
                  <a href="/" aria-label><i class="fa-solid fa-house"></i>
                  ...
                  <header>
                  <h1 id="sections">Entertainment News</h1>
                  </header>
          "##);
      ...
      Err(err) => {
          eprintln!("Error fetching entertainment news: {:?}", err);
          HttpResponse::InternalServerError().body(format!("Failed to fetch entertainment news: {:?}", err))
      }
  }
}